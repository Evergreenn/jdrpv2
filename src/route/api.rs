        ...
        match token {
            Some(t) => {
                ...
                let json: serde_json::Value =
                    serde_json::from_str(f.as_ref()).expect("JSON was not well-formatted");
                let token = decode::<Claims>(
                    &t,
                    &DecodingKey::from_rsa_components(
                        json["keys"][0]["n"].as_str().unwrap(),
                        json["keys"][0]["e"].as_str().unwrap(),
                    ),
...
pub fn testobjectgenerationlol(time: u8) -> Json<Vec<Loot>> {
    ...
    loop {
        ...
        vector.push(generate_loot(None))
    }
    Json(vector)
}

#[get("/api/generationarmor/<time>")]
pub fn generationarmor(time: u8) -> Json<Vec<Loot>> {
    use crate::item_generator::generator::generate_loot;
    use crate::item_generator::resources::ItemType;
    let mut count = 0u8;
    let mut vector = Vec::new();
    loop {
        count += 1;
        ...
        vector.push(generate_loot(Some(ItemType::Armor)))
    }
    Json(vector)
}

#[get("/api/generationconsumable/<time>")]
pub fn generationconsumable(time: u8) -> Json<Vec<Loot>> {
    use crate::item_generator::generator::generate_loot;
    use crate::item_generator::resources::ItemType;
    let mut count = 0u8;
    let mut vector = Vec::new();
    loop {
        ...
        vector.push(generate_loot(Some(ItemType::Consumable)))
    }
    Json(vector)
}
...
pub fn check_caracter_creation(
    token: Token,
    mut u: Json<CaracterStats>,
) -> Result<Json<bool>, Json<Error>> {
    ...
    match s {
        Ok(config) => {
            ...
            if !w.is_empty() {
                Err(Json(Error {
                    code: 16873154,
                    message: String::from("Stats have been altered"),
                }))
            } else {
                ...
                check.push(
                    u.dexterity as i8 - (stats_from_race.dexterity + stats_from_class.dexterity),
                );
                check.push(u.luck as i8 - (stats_from_race.luck + stats_from_class.luck));
                check.push(
                    u.willpower as i8 - (stats_from_race.willpower + stats_from_race.willpower),
                );
                check.push(
                    u.endurance as i8 - (stats_from_race.endurance + stats_from_class.endurance),
                );
                check.push(u.charism as i8 - (stats_from_race.charism + stats_from_class.charism));
                check.push(
                    u.perception as i8 - (stats_from_race.perception + stats_from_class.perception),
                );
                check.push(
                    u.education as i8 - (stats_from_race.education + stats_from_class.education),
                );
